...
import camelCase3 from "lodash/camelCase";
...
var toExpressLikePath = (path3) => path3.replace(/{(.+?)}/g, (_match, p1) => `:${camelCase(p1)}`);
...
function getResIdentifierName(res) {
  ...
  return camelCase2(`get ${res.id}${res.code}Response`);
}
...
function getOperationDefinitions(v3Doc) {
  return Object.entries(v3Doc.paths).flatMap(
    ([path3, pathItem]) => !pathItem ? [] : Object.entries(pathItem).filter((arg) => operationKeys.includes(arg[0])).map(([verb, operation]) => {
      const id = camelCase3(operation.operationId ?? verb + "/" + path3);
      return {
        path: path3,
        verb,
        id,
